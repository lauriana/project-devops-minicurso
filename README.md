...
| `Authentication plugin 'caching_sha2_password' is not supported` | O erro corre porque o MySQL 8+ usa por padrão o plugin de autenticação caching_sha2_password, mas a biblioteca que você está usando no Python está desatualizada ou não reconhece esse método nativamente. Baixar a biblioteca correta |
